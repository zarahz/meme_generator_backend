...
const updateTemplateStatisticViewedAfterCreation = async (memes) => {
    return Promise.all(memes.map(meme => {
        if (meme.template && !meme.template.includes("blob:http://localhost:8080") || !meme.template.includes('data:image/jpeg')) {
...
const updateTemplateStatisticUpvoted = async (memeId) => {
    ...
    if (meme && meme.template && !meme.template.includes("blob:http://localhost:8080") || !meme.template.includes('data:image/jpeg')) {
